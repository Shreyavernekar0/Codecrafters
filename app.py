...
def page_nlp():
    ...
    if st.button(t["extract_info"], type="primary"):
        ...
        if ok:
            if data.get("status") == "success" and data.get("extracted_info"):
                st.success(t["nlp_success"])
                st.json(data.get("extracted_info"))
...
# Show related dataset results if available
                    if data.get("matches"):
                        st.subheader("Related Dataset Entries")
                        ble(data["matches"])
                    else:
                        st.info("No matching entries found in dataset.")
                else:
                    st.error(data.get("message", "Unknown error"))
            else:
